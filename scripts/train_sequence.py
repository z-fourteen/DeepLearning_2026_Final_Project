...
# Fix OpenMP conflict on Windows (libiomp5md.dll duplicate init)
...
class WarmupCosineScheduler(torch.optim.lr_scheduler.LRScheduler):
    ...
    Args:
        ...
        warmup_epochs: Number of linear warmup steps.
        total_epochs: Total training epochs (T_max for cosine phase).
        last_epoch: Index of last epoch (default -1 for fresh start).
    """
    ...
    def __init__(
        ...
        self.total_epochs = max(self.warmup_epochs, total_epochs)
    ...
    def get_lr(self) -> list[float]:
        ...
        # Cosine decay from warmup peak to near-zero
        progress = (self.last_epoch - self.warmup_epochs) / max(
            1, self.total_epochs - self.warmup_epochs
        )
        return [0.5 * base_lr * (1.0 + math.cos(math.pi * progress)) for base_lr in self.base_lrs]
...
def build_scheduler(
    ...
    if scheduler_name == "warmup_cosine":
        ...
        return WarmupCosineScheduler(optimizer, warmup_epochs=warmup_epochs, total_epochs=max_epochs)
...
def main() -> None:
    ...
    model = _MODEL_REGISTRY[model_name](num_features=num_features, config=model_config)
